...
use App\DataTables\MotorCommercialDataTable;

class MotorCommercialController extends Controller
{
	//=================================================================

	public function index(MotorCommercialDataTable $dataTable)
	{
		return $dataTable->render('admin/motor_commercial/index');
	}
	...
	public function add(Request $request)
	{
		...
		$data['sub_category'] = Policysubcategory::where('category_id',$data['category']->id)
									->where('name','LIKE','%Motor Commercial%')
									->where('status','1')
									->first();
		$data['customers'] = Customer::where('status','1')->get();
		
		return view('admin/motor_commercial/add',$data);
	}
	...
	public function save(Request $request)
	{
		try {
			...
			if ($validator->fails()) { 
				return redirect('admin/motor_commercial/add')
	                        ->withErrors($validator)
	                        ->withInput();
			} else {	        
	         	$data = new MotorCommercial();
	         	//================================================
				...
				return redirect('admin/motor_commercial/index');
			}
		} catch (\Exception $e) {
	...
	{
		$data = array();
		$data['result'] = MotorCommercial::where('id',$id)->first();
		$data['category'] = Policycategory::where('name','LIKE','%motor%')->first();
		$data['sub_category'] = Policysubcategory::where('category_id',$data['category']->id)
		...
		return view('admin/motor_commercial/edit',$data);
	}
	...
	{
		try {
			MotorCommercial::where('id', $id)->delete();
