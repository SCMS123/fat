...
use App\DataTables\MotorPrivateDataTable;

class MotorPrivateController extends Controller
{
	//=================================================================

	public function index(MotorPrivateDataTable $dataTable)
	{
		return $dataTable->render('admin/motor_private/index');
	}
	...
	public function add(Request $request)
	{
		...
		return view('admin/motor_private/add',$data);
	}
	...
	public function save(Request $request)
	{
		try {
			...
			if ($validator->fails()) { 
				return redirect('admin/motor_private/add')
	                        ->withErrors($validator)
	                        ->withInput();
			} else {	        
	         	$data = new MotorPrivate();
	         	//================================================
				...
				return redirect('admin/motor_private/index');
			}
		} catch (\Exception $e) {
	...
	{
		$data = array();
		$data['result'] = MotorPrivate::where('id',$id)->first();
		$data['category'] = Policycategory::where('name','LIKE','%motor%')->first();
		$data['sub_category'] = Policysubcategory::where('category_id',$data['category']->id)
									->where('name','LIKE','%cycle%')
									->where('status','1')
		...
		return view('admin/motor_private/edit',$data);
	}
	...
	{
		try {
			MotorPrivate::where('id', $id)->delete();
