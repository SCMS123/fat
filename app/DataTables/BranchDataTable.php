...
class BranchDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $checkStatusAction = Role::where('name_slug','branches')->whereRaw("find_in_set('".$status_action->id."',action_id)")->first();
        $roles = Role::where('name_slug','branches')->first();
            ...
            ->editColumn('action', function($row) use ($roles) {
                ...
                foreach ($action_ids as $key => $action_id) {
                    ...
                    if ($action->action_slug == 'edit' || $action->action_slug == 'delete' || $action->action_slug == 'view') {
                        $btn .= '<a href="'.route("branches.$action->action_slug",$row->id).'" class="btn btn-'.$action->class.' btn-sm" data-placement="top" data-original-title="'.$action->action_title.'"><i class="'.$action->icon.'"></i>'.$action->action_title.'</a>&nbsp;';
                    }
                }
