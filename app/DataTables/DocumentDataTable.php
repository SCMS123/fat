...
class DocumentDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $checkStatusAction = Role::where('name_slug','documents')->whereRaw("find_in_set('".$status_action->id."',action_id)")->first();
        $roles = Role::where('name_slug','documents')->first();
        ...
        return datatables()
            ...
            ->editColumn('image', function($row) {
                if (!empty($row->image)) {
                    $url = url('/public/admin/clip-one/assets/documents/original').'/'.$row->image;
                    return '<ul class="list-inline"><li class="list-inline-item"><a href="'.$url.'" download="'.$row->image.'">Download Attachment</a></li></ul>';
                }else{
                ...
                foreach ($action_ids as $key => $action_id) {
                    ...
                    if (!empty($checkPermission) || Auth::user()->user_type == 'admin') {
                        if ($action->action_slug == 'edit' || $action->action_slug == 'delete' || $action->action_slug == 'view' || $action->action_slug == 'password') {
                            $btn .= '<a href="'.route("documents.$action->action_slug",$row->id).'" class="btn btn-'.$action->class.' btn-sm" data-placement="top" data-original-title="'.$action->action_title.'"><i class="'.$action->icon.'"></i>'.$action->action_title.'</a>&nbsp;';
                        }
                    }
                }
                return $btn;
